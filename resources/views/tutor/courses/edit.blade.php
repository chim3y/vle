...
{!! Form::model($course,['files'=>'true','method'=>'PATCH', 'action'=>['tutor\CoursesController@update', $course->id]]) !!}
...
<div class="col-sm-6">
  ...
  <option value="" selected> Please select Programme</option>
    @foreach($programmes as $item)
      ...
      <option value="{{$item->id}}" {{ ($item->id == $c_p->programme_id) ? 'selected=selected' : '' }}>

      {{$item->programme_name}}</option>
    @endforeach
  </select>
...
<div class="col-sm-6">
  ...
  <option value="" selected> Please select Semester </option>
    @foreach($semesters as $item)
      <option value="{{$item->id}}" {{ ($item->id == $c_p->semester_id) ? 'selected=selected' : '' }}>
