...
<div style="height: 90px;
  ...
  @if( $userpercentage <40 )
   <h1> <i class="fa fa-hand-o-right" aria-hidden="true"></i> <a href="{{ route('student.quiz.show', [$quiz->id, str_slug($quiz->quiz_name)]) }}" style="color: black;" target="_self">  PLEASE TRY AGAIN </a> </h1>
...
<br/>
<strong> ATTEMPTED BY: </strong> {{ ucfirst(trans(Auth::guard('web')->user()->name))}}
<br/>
...
<h3>Question: <strong>  {!!$question->question!!}  </strong> </h3>
 ...
 <input type="radio"  value="{{$ans->id}}" {{ ($ans->id == $answer->where('question_id', $ans->question_id)->where('is_correct', 1)->first()->id) ? 'checked=checked' : '' }}> <span> {{$ans->answer}} </span> <br/>
  ...
  <strong> Your Answer: </strong>  
    @if(!empty($useranswer->where('question_id', $ans->question_id)->first()))
   ...
   @elseif(empty($useranswer->where('question_id', $ans->question_id)->first()))
   ...
   <br/>
    ...
    @elseif($question->type== "fill-in-blanks")
     ...
     @foreach($question->answer as $ans)
      <option value="{{$ans->id}}" {{ ($ans->id == $answer->where('question_id', $ans->question_id)->where('is_correct', 1)->first()->id) ? 'selected' : '' }} >{{$ans->answer}} </option>
     @endforeach
     </select>
    <br/>
      <div class="panel-footer col-sm-12" style="background-color:#ffbb33"> 
 ...
 No answer choosen 
   ...
   @endif
    ...
    {{$answer->where('question_id', $ans->question_id)->where('is_correct', 1)->first()->answer}}
     ...
     <input type="checkbox" value="{{$ans->id}}" {{ ($ans->id == $answer->where('question_id', $ans->question_id)->where('is_correct', 1)->first()->id) ? 'checked=checked' : '' }}> {{$ans->answer}} <br/>
       @endforeach
       <br/>
 <div class="panel-footer col-sm-12" style="background-color:#ffbb33"> 
 <strong> Your Answer: </strong>
