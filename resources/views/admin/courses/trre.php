...
<div class="col-sm-4">
<h5 style="display:inline-block"> <a href="{{ route('admin.quiz.show', [$quiz->id, str_slug($quiz->quiz_name)]) }}" style="color: black;" target="_self"> <strong>  {{ \Illuminate\Support\Str::upper($quiz->quiz_name ) }} </strong> </a></h5> 
</div>

<div class="col-sm-4"> 
    <button class="btn btn-link pull-right"  style="color:black; font-size:14px">  <a  href="{{ URL::route('admin.contents.quizes.edit', array('contentId'=> $content->id, 'id'=> $quiz->id )) }}" target="_self" style="color:black"> edit </a>  </button> 
 

{!! Form::open(['action' => ['Admin\QuizesController@destroy', 'id'=> $quizes->id], 'method' => 'delete', 'style'=>' display:inline-block']) !!}
    <button class="btn btn-link pull-right" style="color: black; font-size:14px"> delete </button>
 {!! Form::close() !!}   
</div>
</div> 
